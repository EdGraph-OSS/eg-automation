...
def main() -> None:
    ...
    nde_endpoints = client.get_edfi_instance_endpoints(nde_external_instance_id, school_year)
    nde_api_client = client.get_edfi_instance_application(
    ...
    if not state.source_connection_id:
        existing = client.search_datasync_connections(source_name)
        if existing.has_elements():
            ...
            created = client.create_datasync_connection(
                CreateEdFiAdminConnectionRequest(
                    tenant_id=tenant_id,
                    name=source_name,
    ...
    if not state.destination_connection_id:
        test_result = client.test_datasync_connection(
            EdFiAdminTestConnectionRequest(
                connection_id=None,
                provider_id=EDFI_CONNECTION_PROVIDER_ID,
                connection_type_id=EDFI_CONNECTION_TYPE_ID,
        ...
        existing = client.search_datasync_connections(dest_name)
        if existing.has_elements():
    ...
    if not state.job_id:
        job: DataSyncJob = client.create_datasync_job(
